...
describe('applyReward — 보상 단일 진입점 (긴급 안정화 F)', () => {
  ...
  it('보상표대로 적립된다', () => {
    ...
    ).toMatchObject({ applied: true, xp: 25, points: 5 })
    expect(
      applyReward({ id: 'c1', sessionId: 's1', type: 'session_completed' }),
    ).toMatchObject({ xp: 60, points: 30 })
    expect(
      applyReward({ id: 'st1', sessionId: 's1', type: 'stretch_completed' }),
    ).toMatchObject({ xp: 10, points: 10 })
    expect(
      applyReward({ id: 'g1', sessionId: 's1', type: 'goal_completed' }),
    ).toMatchObject({ xp: 15, points: 10 })

    expect(useProgressionStore.getState().xp).toBe(25 + 60 + 10 + 15)
    expect(useProgressionStore.getState().points).toBe(5 + 30 + 10 + 10)
  })
  ...
  it('recovery XP 반영은 세션당 최대 5회 — 이후에는 0 지급(capped)', () => {
    ...
    expect(useProgressionStore.getState().xp).toBe(25 * MAX_REWARDED_RECOVERIES)
  })
  ...
  it('같은 스트레칭 완료 이벤트 id 는 1회만 보상된다 (Gate 1)', () => {
    ...
    expect(first).toMatchObject({ applied: true, xp: 10, points: 10 })
    expect(second.applied).toBe(false)
  ...
  it('세션 화면 집계(sessionXp)는 recovery·완료 보상에서만 오른다', () => {
    ...
    expect(useGameStore.getState().sessionXp).toBe(25)
    expect(useGameStore.getState().sessionPoints).toBe(5)
  })
})
